...
def carica_dati_reali(file_list):
    """Carica i dati reali con gestione degli errori"""
    df_list = []
    for file_path in file_list:
        try:
            ...
            if "Italia" not in df_temp.columns:
                ...
                # Prova a cercare colonne simili
                colonne_disponibili = df_temp.columns.tolist()
                print(f"   Colonne disponibili: {colonne_disponibili}")
                continue
...
def main():
    ...
    try:
        print("🚀 Avvio sistema di previsione prezzi elettrici con dati reali...")
        
        # === 1. CARICA I FILE EXCEL REALI ===
        file_list = [
            "/Users/emanuelediluzio/rob_dif/2022_MGP.xlsx",
            "/Users/emanuelediluzio/rob_dif/2023_MGP.xlsx",
            "/Users/emanuelediluzio/rob_dif/2024_MGP.xlsx",
            "/Users/emanuelediluzio/rob_dif/2025_MGP.xlsx"
        ]
        
        df = carica_dati_reali(file_list)
        ...
        # === 6. SALVA IN EXCEL ===
        output_file = "previsione_italia_trading_dati_reali.xlsx"
        ...
        # Statistiche sui dati reali
        print(f"\n📊 STATISTICHE DATI REALI:")
        print(f"   • Periodo dati: {df['Data'].min().strftime('%d/%m/%Y')} - {df['Data'].max().strftime('%d/%m/%Y')}")
        print(f"   • Prezzo medio storico: {df['Prezzo'].mean():.2f} €/MWh")
        print(f"   • Prezzo massimo storico: {df['Prezzo'].max():.2f} €/MWh")
        print(f"   • Prezzo minimo storico: {df['Prezzo'].min():.2f} €/MWh")
        
    except Exception as e:
        print(f"❌ Errore durante l'esecuzione: {e}")
        print("💡 Verifica che i file Excel siano presenti e abbiano il formato corretto")
