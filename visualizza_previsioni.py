...
def visualizza_previsioni():
    """Visualizza le previsioni con grafici"""
    
    # Carica i dati
    df = pd.read_excel('previsione_italia_trading_ottimizzata.xlsx')
    df['Data'] = pd.to_datetime(df['Data'])
    df['DataOra'] = df['Data'] + pd.to_timedelta(df['Ora'] - 1, unit='h')
    
    # Crea figura con subplot
    fig, axes = plt.subplots(2, 2, figsize=(15, 12))
    fig.suptitle('📊 Previsioni Prezzi Elettrici Italia - Prossimi 7 Giorni', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('previsioni_grafico.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Stampa statistiche dettagliate
    print("\n" + "="*60)
    print("📊 STATISTICHE DETTAGLIATE PREVISIONI")
    print("="*60)
